// components/CollectionSpotlight.tsx
...
const CollectionSpotlight: React.FC<CollectionSpotlightProps> = ({ onClose }) => {
  return (
    <div className="container mx-auto p-4">
      <h2 className="text-6xl font-bold mb-10 text-center ">Collection Spotlight</h2>
      <p className="text-center m-10 p-5">Discover extraordinary moments with our Spotlight Collection metatickets—exclusive access to premium events for an unforgettable experience. Grab yours today!</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-10 m-20">
        {collectionsData.map((collection) => (
          <div key={collection.id} className="bg-white-100 p-3 shadow-2xl relative">
            <img src={collection.image} alt={collection.name} className="w-full mb-2 " style={{ maxWidth: "100%", height: "auto" }} />
            <div className='flex justify-between'>
            <span className="absolute left-0 -ml-5 h-10 w-10 bg-gray-100 flex items-center justify-center rounded-full"></span>
            <span className="border-t border-dashed border-gray-400 ml-5 mr-5 mt-5 block w-full h-10"></span>
            <span className="absolute right-0 -mr-5 h-10 w-10 bg-gray-100 flex items-center justify-center rounded-full"></span>
            </div>
            <p className="font-semibold text-center text-xl">{collection.name}</p>
            <div className="flex gap-1 mx-auto p-2 justify-center items-center text-sm" >  
            <div >{collection.date}</div> |
            <div >{collection.day}</div> |
            <div >{collection.time}</div>
            </div>
            <p className='text-center p-4 text-gray-600'>{collection.address}</p>
