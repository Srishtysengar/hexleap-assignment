...
const SportsContainer: React.FC = () => {
  ...
  return (
    <div className="container mx-auto p-4 ">
      <div className="text-2xl font-bold mb-4">Sports</div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {/* Your cards go here */}
        {players.map((player) => (
          <div key={player.id} className="bg-white-100 p-3 rounded-lg shadow-2xl">
            <img src={player.image} alt={player.name} className="w-full mb-2 " style={{ width: "1080px", height: "400px" }}/>
            <p>{player.name}</p>
            <div className='bg-gray-100 flex justify-between mt-2 text-sm flex p-3'>
            <div className="flex justify-between mt-2 text-sm flex-col">
                <span className="text-gray-600">Total Events</span>
                <span className="font-bold text-gray-800">{`${player.totalEvents} Events`}</span>
            </div>
            <div className="flex justify-between mt-2 text-sm flex-col">
                <span className="text-gray-600">Sports</span>
                <span className="font-bold text-gray-800">{player.sports}</span>
            </div>
            </div>
          </div>
        ))}
        <div className='bg-white-100 p-3 shadow-2xl relative'>
            ...
            <h2 className='text-xl mt-10 font-semibold p-1'> Advertisement title</h2>
            <p className='mt-2 text-sm p-1'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Magnam et eligendi saepe illum consequatur! Suscipit iste sapiente corrupti. Sunt eos dolores cupiditate delectus repudiandae facere illum mollitia obcaecati laboriosam esse?</p>
        </div>
      </div>
